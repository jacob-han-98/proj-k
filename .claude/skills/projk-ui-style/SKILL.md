...
**표시 규칙**:
- 카드 라벨은 **원본 라벨**(`origin_label`) — 내부 가공 경로(`packages/xlsx-extractor/output/...`) 를 직접 표시하지 말 것.
  - Excel: `워크북명.xlsx / 시트명 시트`
  - Confluence: `Confluence / 공간 / … / 페이지`
- 클릭 링크는 `origin_url` 우선 (Confluence viewpage URL). Excel 은 링크 없음 — `<div>` 로 렌더.
- `title` 속성(툴팁)으로 내부 path 를 노출하여 디버깅은 가능하게 유지.
- **Confluence 검색 링크(`wiki/search?text=...`) 사용 금지** — 반드시 `pageId` 기반 직접 링크.
...
### 진행 타임라인 (Claude Code 스타일 collapsible)
```css
.progress-panel { /* <details> */ }
.progress-panel > summary { /* 클릭 접힘/펼침, ▶/▼ 마커 */ }
.progress-body { border-left: 2px solid var(--border-color); padding-left: 12px; }
.tool-entry { 개별 tool 호출: 라벨 + 결과 요약 }
.tool-label { monospace, 원본 input 유지 }
.tool-summary { 라벨 옆 한 줄 요약 ("Found 6 files", "67 라인 (1,262 자)") }
.thinking-entry { 접힘된 💭 사고 누적 }

/* 로딩 중에는 살아있음을 shimmer + border pulse 로 표시 */
.loading .progress-head { background-clip: text; color: transparent;
  animation: progressShimmer 2.2s ease-in-out infinite; }
.loading .progress-panel { animation: progressBorderPulse 2.4s ease-in-out infinite; }
```

**UX 원칙**: 에이전트가 답변 작성 중일 때도 반드시 **시각적 움직임**이 있어야 한다 (텍스트 변경 없는 구간에서도 shimmer/pulse로 진행 중임을 표시). 정지 UI 로 사용자가 프리즈를 의심하게 만들지 말 것.
