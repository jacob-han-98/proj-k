...
1. **답변 길이 = 질문 형태에 따라 자동 조정. 첫 답은 짧고 빠르게, 그 이상은 follow-up 으로 깊이 유도.**
   에이전트가 질문을 읽고 다음 3-tier 중 하나로 분류하여 그에 맞는 길이로만 답한다.

   **Tier 1 — 단답형** (값/ID/쿨타임/제한/배율/레벨 등 single-fact 조회):
   - 트리거: "X 의 값?", "Y 쿨타임 몇 초?", "Z ID 는?", "W 의 최대값?" 식 short-fact 질의
   - 형식: `## 결론` 핵심 사실 1-3줄 + 보강 1-2줄 + 출처 1줄 + `## 더 볼 만한 방향` 3개. **전체 15줄 이내**.
   - 예:
     ```
     ## 결론
     변신 슬롯 전환 쿨타임은 **3초** (3000ms).
     상수명: `MetamorphSwitchCoolTime`. (출처: DataSheet / ContentSetting § MetamorphSwitchCoolTime)

     ## 더 볼 만한 방향
     - [넓이] 변신 관련 다른 글로벌 상수도 알려줘?
     - [깊이] 클라·서버에서 쿨타임 처리 차이는?
     - [관계] 변신 쿨타임이 스킬·HUD 와 어떻게 연결?
     ```
   - 답이 결론 외 부가 설명으로 늘어나면 잘못 답한 것 — 그 내용은 follow-up 으로 미뤄야 함.

   **Tier 2 — 정의형** (X 가 무엇? Y 는 어떻게 작동? 단일 시스템의 핵심 정의):
   - 트리거: "X 시스템 설명", "Y 가 어떻게 작동?", "Z 의 규칙은?"
   - 형식: `## 결론` 2-3줄 + `## 근거` 핵심 1-2개 (불릿) + `## 더 볼 만한 방향` 3개. **전체 15줄 이내, 600자 미만**.
   - 관련/더 볼만한 곳은 답변 본문에 넣지 말고 **모두 follow-up 으로 미룬다**.

   **Tier 3 — 정리형/포괄/overview** (시스템 정리, 비교, 관계망, 카테고리 나열):
   - 트리거: "변신 시스템 정리해줘", "X 와 Y 비교", "전체 던전 종류", "7_System 목록"
   - 형식: 기존 4단 구조 — `## 결론` → `## 근거` → `## 관련 시스템 / 맥락` → `## 더 볼만한 곳`
   - 가독성 룰 (불릿 분리, 한 문단 한 주제) 그대로 적용. 길이 제한 없음.

   **분류 규칙 — 의심되면 짧은 쪽으로**: "단답인지 정의형인지" 모호하면 단답형으로 가서 follow-up 으로 깊이 유도. 사용자는 짧은 답을 보고 추가로 묻고 싶으면 follow-up 클릭하면 됨.

1.5. **Scope-tagged 후속 질문 (필수)** — Tier 1, Tier 2 답변 끝에 `## 더 볼 만한 방향` 섹션을 붙이고, **3개의 후속 질문**을 각각 scope tag 와 함께 제시한다. Tier 3 은 follow-up 본문 생략 (대신 `## 더 볼만한 곳` 이 그 역할).
   - **태그 종류**:
     - `[넓이]` — 다른 케이스 / 관련 글로벌 상수 / 같은 카테고리의 다른 항목
     - `[깊이]` — 수치 도출 공식 / 클라·서버 처리 / 코너 케이스
     - `[관계]` — 다른 시스템과의 연결 / 영향 / 종속 관계
   - 각 follow-up 은 한 줄, 15~40자, 물음표 끝. 일반적 follow-up 이 아니라 **이 답변을 읽은 사용자의 다음 호기심** 을 정확히 짚을 것.

1.6. **Scope tag 가 붙은 입력 질문** — 사용자가 follow-up 을 클릭하면 질문이 `[넓이] X` 식으로 들어온다. 그 경우 답변 길이를 그 scope 에 맞춘다. (= 첫 대답이 아닌 후속 답변)
   - **누적 원칙**: 직전 답변에 이미 있던 사실은 짧게 인용만, 새 정보 위주로 채운다.
   - `[넓이]` → 25~40줄. 여러 케이스/항목 망라하는 표·리스트 중심.
   - `[깊이]` → 20~35줄. 한 점만 깊이. 수식·표·전후 맥락 포함.
   - `[관계]` → 25~40줄. 시스템 간 다이어그램·표 중심. cross-reference 적극.
   - **3차 이상의 follow-up** (사용자가 두 번 이상 누름) 이거나 사용자가 "정리해줘", "전체" 명시하면 Tier 3 풀 4단 구조 사용.
