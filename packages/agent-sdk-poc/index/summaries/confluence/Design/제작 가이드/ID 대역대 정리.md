# ID 대역대 정리 (요약)

> 출처: Confluence / Design / 제작 가이드 / ID 대역대 정리
> 원본: packages/confluence-downloader/output/Design/제작 가이드/ID 대역대 정리/content.md

## 한 줄 설명
Project K 게임 내 모든 엔티티(몬스터, NPC, 퀘스트, 오브젝트 등)의 ID 대역대 범위와 명명 규칙을 정의하는 문서.

## 핵심 용어
- ID 대역대
- Reward
- Volume
- Monster Volume
- NPC Volume
- TeleportVolume
- SpawnVolume
- RespawnVolume
- Object Volume
- Quest Volume
- Quest
- QuestObject
- Npc
- Monster
- MonsterReward
- QuestReward
- MonsterSpawn
- NpcSpawn
- WorldTerritory
- Effect
- WorldClass
- QuestObject
- QuestObjective
- QuestClass
- NpcClass
- MonsterClass
- 메인 퀘스트
- 서브 퀘스트
- 주간 퀘스트
- 일간 퀘스트
- 퀘스트 Npc
- 상인 및 기능 Npc
- 필드 Monster
- 정령의 탑 Monster
- 던전 Monster
- 시네마틱 ID

## 숫자/상수/공식
- Reward: 8자리
- Volume: 8자리, 범위 10000001~999999999
- Quest: 8자리
  - 메인 퀘스트: 81000000~81999999
  - 서브 퀘스트: 82000000~82999999
  - 주간 퀘스트: 83000000~83999999
  - 일간 퀘스트: 84000000~84999999
- QuestObject: 8자리, 범위 70000000~79999999
- Npc: 8자리
  - 퀘스트 Npc: 30000000~30999999
  - 상인 및 기능 Npc: 31000000~31999999
- Monster: 6자리 또는 8자리
  - 필드 Monster: 6자리, 범위 10000~999999
  - 정령의 탑 Monster: 8자리, 범위 10000000~10099999
  - 던전 Monster: 8자리, 범위 20000000~20099999
- QuestObject Volume ID 규칙: QuestObject ID 1단위를 0~9까지 사용

## 참조 시스템
- (없음)

## 주요 섹션
- ID 대역대 정리 (표)
- 몬스터 볼륨 ID 규칙
- Quest ID 규칙
- Quest Volume ID 규칙
- QuestObject ID 규칙
- QuestObject Volume ID 규칙
- Npc ID 규칙
- Monster ID 규칙
- 시네마틱 ID
