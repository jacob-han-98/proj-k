...
방금 제공된 질문과 답변을 바탕으로, 사용자가 답변을 읽은 뒤 자연스럽게 이어서 궁금해할 만한 **scope-tagged 후속 질문 3개**를 제안합니다.

각 질문은 다음 3가지 scope 중 하나를 정확히 1개씩 커버해야 합니다:
- `[넓이]` — 같은 카테고리의 다른 항목·케이스, 관련 글로벌 상수, 같은 시스템 안의 다른 측면
- `[깊이]` — 수치 도출 공식, 클라·서버 처리 차이, 코너 케이스, 답변에 등장한 한 점의 내부 동작
- `[관계]` — 다른 시스템과의 연결, 종속, 영향, cross-system 흐름

규칙:
1. 답변 내용과 직접 연결되는 맥락적 질문이어야 합니다(단순 재질문/요약 금지).
2. 한국어 · 한 문장 · 15~40자 · 물음표로 끝냅니다. 앞에 scope 태그 prefix 필수: `[넓이] ...?`, `[깊이] ...?`, `[관계] ...?`.
3. 답변에서 열려있는 부분(언급만 되고 구체 설명 없는 주제, 반대 케이스, 연관 시스템, 수치/밸런스 근거)을 파고드세요.
4. 이미 답변이 충분히 설명한 내용을 다시 묻지 마세요.
5. **정확히 3개**, scope 태그가 [넓이]·[깊이]·[관계] 각 1개씩 되도록 균형. 이 답변에 [관계] 가 어색하면 [넓이] 또는 [깊이] 추가 1건으로 대체 가능 (단 태그는 명시).
6. JSON 배열만 반환하세요: ["[넓이] 질문1?", "[깊이] 질문2?", "[관계] 질문3?"]. 다른 텍스트·설명·코드블록 금지.
"""
