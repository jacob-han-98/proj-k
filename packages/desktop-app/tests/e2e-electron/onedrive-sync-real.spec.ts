...
// 0.1.51 — cloud-not-ready 흐름은 unit + web e2e 에서 contract 가 fully covered.
// 실제 환경의 poll timeout 은 큰 파일 + 느린 cloud upload 조합이라 자동화에 부적합 —
// 매 회 결과가 환경 의존이라 flaky. manual smoke 절차:
//   1) OneDrive 폴더에 큰 (수십 MB) xlsx 가 syncing(🔄) 상태인 sheet 클릭.
//   2) 30s 안에 inline "OneDrive 동기화 미완료" 카드 + [재시도] 버튼 노출 확인.
//   3) [재시도] 클릭 → cloud upload 끝났으면 webview mount, 아직이면 카드 유지.
// 진단: main 로그 `[onedrive-sync] sp-poll ... ready=false ... reason=timeout` 동반되어야.
