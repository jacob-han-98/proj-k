...
test('⚙ 토글 → settings 뷰로 swap → preset fetch + textarea prefill + back 복귀', async ({ page }) => {
  ...
  // 2026-05-13: modal 로 전환. modal backdrop + container 위에 떠 있고,
  // 아래 picker 는 그대로 mount (visible 일 수 있지만 backdrop 으로 가려짐).
  await expect(page.getByTestId('assistant-prompt-settings')).toBeVisible();
  await expect(page.getByTestId('assistant-prompt-settings-backdrop')).toBeVisible();

  // 탭 2개 (요약 / 리뷰) 노출 + summary 가 default active.
  await expect(page.getByTestId('assistant-prompt-settings-tab-summary')).toBeVisible();
  await expect(page.getByTestId('assistant-prompt-settings-tab-review')).toBeVisible();
...
test('override 가 preset 과 동일하면 저장 시 localStorage 삭제 + 메타 "preset 사용 중"', async ({ page }) => {
  ...
  // review 탭으로 전환 (default 는 summary).
  await page.getByTestId('assistant-prompt-settings-tab-review').click();
...
test('preset fetch 실패 시 — error 배너 + textarea 비활성, 기존 override 는 표시', async ({ page }) => {
  ...
  // summary 탭 (default active) — error + 저장된 override 가 textarea 에 그대로.
  ...
  // review 탭으로 전환 — review 측 error + 빈 textarea.
  await page.getByTestId('assistant-prompt-settings-tab-review').click();
