...
test('어시스턴트 버튼은 Confluence 페이지 선택 시 doc-header 에 노출, sheet 에선 노출 안 됨', async ({ page }) => {
  await page.goto('/');

  // 시작 (selection 없음) — 버튼 없음
  await expect(page.getByTestId('confluence-assistant')).toHaveCount(0);
  ...
  await tree.getByText('시스템 디자인', { exact: true }).click();
  await tree.getByText('전투', { exact: true }).click();
  await expect(page.getByTestId('confluence-assistant')).toBeVisible();
});

test('어시스턴트 버튼 클릭 → mode picker 빈 상태 노출 (수동 시작) — 백엔드 호출 X', async ({ page }) => {
  // P0: 수동 시작 정책 — 버튼만 누르면 mode='pick' 으로 패널이 열리고, 사용자가 모드 칩을
  // 누르기 전엔 review_stream 등 백엔드 호출이 일어나지 않음.
  ...
  // mode picker 빈 상태 노출. 3개 칩 모두 보이고 review 만 활성, 나머지는 disabled.
  await expect(page.getByTestId('mode-picker-empty')).toBeVisible();
  await expect(page.getByTestId('mode-pick-review')).toBeEnabled();
  await expect(page.getByTestId('mode-pick-summary')).toBeDisabled();
  await expect(page.getByTestId('mode-pick-agent')).toBeDisabled();

  // review-card 는 아직 mount 안 됨 (사용자가 review 칩 안 눌렀으므로).
...
test('리뷰 streaming → result.data.review (WSL 포맷, JSON 문자열) → ReviewCard 데이터 렌더', async ({ page }) => {
  ...
  // A5: perspective 가 색상 chip 으로 표시 — 텍스트만 (대괄호 없음).
  await expect(card.locator('.review-perspective')).toContainText('기획팀장');
...
test('리뷰 — result.data.review 가 ```json ... ``` 마크다운 펜스로 감싸여도 파싱', async ({ page }) => {
  ...
  // A5: perspective chip — 텍스트만 (대괄호 없음).
  await expect(card.locator('.review-perspective')).toContainText('기획팀장');
