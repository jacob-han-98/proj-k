...
import QualityCriteriaPanel from './QualityCriteriaPanel'
    ...
    <div className="layout">
      ...
      <aside className={`sidebar glass ${sidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="theme-selector" style={{ margin: 0 }}>
          <button className={`theme-btn ${adminTab === 'conversations' ? 'active' : ''}`} onClick={() => setAdminTab('conversations')}>대화</button>
          <button className={`theme-btn ${adminTab === 'quality' ? 'active' : ''}`} onClick={() => setAdminTab('quality')}>품질</button>
          <button className={`theme-btn ${adminTab === 'pipeline' ? 'active' : ''}`} onClick={() => setAdminTab('pipeline')}>파이프라인</button>
        </div>
        {adminTab === 'conversations' && <div className="admin-stats">
          {convList.length}개 대화 {loading && '(로딩...)'}
          {error && <span style={{color: '#ef4444'}}> (연결 실패)</span>}
        </div>}
        {adminTab === 'conversations' && <div className="sidebar-section">
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="conv-meta">
                    {conv.turn_count}턴 · {conv.last_model.replace('claude-', '')} · {formatTime(conv.updated_at)}
                  </div>
                </div>
              </div>
            ))}
              ...
              {detail && (
                <div className="admin-conv-header glass">
                  ...
                  <div className="admin-conv-meta-row">
                    ...
                    {detail.turns[0] && <span>모델: {detail.turns[0].model}</span>}
                  </div>
                </div>
              )}
              {/* 메시지 (Chat과 동일한 UI) */}
