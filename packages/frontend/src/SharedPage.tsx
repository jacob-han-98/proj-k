...
import type { ConversationDetail, Source, Proposal } from './api'
...
function SharedPage() {
  ...
  const messages = detail.turns.flatMap(turn => [
    { role: 'user' as const, content: turn.question, sources: undefined as Source[] | undefined, proposals: undefined as Proposal[] | undefined, model: turn.model, tokens: turn.total_tokens, seconds: turn.api_seconds },
    { role: 'assistant' as const, content: turn.answer, sources: turn.sources, proposals: (turn as any).proposals as Proposal[] | undefined, model: turn.model, tokens: turn.total_tokens, seconds: turn.api_seconds },
  ])
      ...
      <div className="shared-content">
        <div className="shared-meta">
          ...
          <div className="shared-meta-row">
            ...
            {detail.turns[0] && <span>{detail.turns[0].model.replace('claude-', '').replace('-', ' ')}</span>}
          </div>
        </div>
            ...
            <div key={idx} className={`message-wrapper ${msg.role}`}>
              <div className={`message glass ${msg.role}`}>
                ...
                {msg.proposals && msg.proposals.length > 0 && (
                  <ProposalView proposals={msg.proposals} conversationId={id} />
                )}
