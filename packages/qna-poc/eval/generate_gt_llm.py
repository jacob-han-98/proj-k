...
5. **질문 품질 규칙** (매우 중요):
   ...
   - 답변은 기본 규칙/원칙을 먼저 설명하고, 특정 케이스는 그 다음에 부연하세요.
     - BAD: "Case.2에 따르면 중형을 건너뛰고 대형으로 대체됩니다" (지엽적)
     - GOOD: "기본 규칙은 상급 물약으로 자동 대체됩니다. 소/대만 소지 시(Case.2) 중형을 건너뛰고 대형으로 갑니다." (기본→케이스 순서)
...
def main():
    ...
    if not args.no_traps:
        ...
        print(f"{'='*60}")
        trap_count = 45 if args.sample == 0 else max(3, args.sample)
