...
## 현재 상태: Stage 1-4 구현 완료, Verification 미착수
...
### Stage 4: Synthesize (구현 완료, 2026-03-08)

**구현**: `synthesize.py` — Vision + Parse 결과 합성 → `_final/content.md` + `_final/images/`

**핵심 기능 (구현됨)**:
1. **타일 경계 중복 제거**:
   - 타일 섹션 헤더 제거 (`# SheetName - Section N/M`)
   - `(계속)` 접미사 제거 (모든 헤딩에서)
   - 연속 중복 헤딩 정리 (부모 컨텍스트 반복 제거)
   - 불완전 잘림 섹션 제거 (< 8줄의 짧은 중복 섹션)
2. **Parse 보정 통합**: OOXML Mermaid 검증/보정 자동 적용
3. **메타데이터 헤더**: 원본 파일, 시트명, 변환일, 파이프라인 버전
4. **서브 이미지 정리**: 참조되는 이미지만 `_final/images/`에 복사, 미참조 건너뛰기
5. **Dangling 참조 처리**: dedup으로 제거된 섹션의 이미지 참조 → 텍스트 설명으로 교체

**전체 처리 결과 (PK_변신 및 스킬 시스템.xlsx)**:

| 시트 | Lines | Bytes | Dedup | Images | Parse |
|------|-------|-------|-------|--------|-------|
| UI_변신_강화 | 300 | 22,403 | -41 | 8 | +1 edge |
| UI_변신_기본 | 316 | 23,551 | -33 | 14 | - |
| UI_변신_컬렉션 | 27 | 694 | 0 | 0 | - |
| UI_변신_합성 | 343 | 27,081 | -26 | 16 | verified |
| UI_변신_확정 | 30 | 768 | 0 | 0 | - |
| UI_스킬 | 238 | 23,683 | -23 | 9 | - |
| 변신 | 833 | 35,441 | -30 | 0 | verified |
| 스킬 | 518 | 27,934 | -7 | 7 | verified |
| 스킬 기능 리스트 | 748 | 37,727 | -39 | 17 | +2 edges |
| 아트_연출 리스트 | 38 | 1,705 | -1 | 0 | - |
| 주요 정의 | 91 | 5,607 | -1 | 0 | - |
| 테이블 정보 | 29 | 978 | -1 | 0 | - |
| 히스토리 | 134 | 22,158 | -13 | 0 | - |
| **합계** | **3,804** | **256,271** | **-225** | **71** | |

- 총 출력: 3,804줄, 250KB, 71개 서브 이미지
- dedup: 225줄 제거 (타일 헤더, 컨텍스트 반복, 잘림 섹션)
- Parse 보정: UI_변신_강화 +1 edge, 스킬 기능 리스트 +2 edges
- 소요 시간: <1초 (API 호출 없음, 순수 텍스트 처리)
