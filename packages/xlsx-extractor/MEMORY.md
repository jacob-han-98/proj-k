...
- [x] 플로우차트 크롭 재분석 파이프라인 구현 (2026-03-08)
  - 3단계: has_flowchart() → locate_flowchart_bbox() → crop + reprocess_flowchart() → replace_mermaid_block()
  - locate: 전용 Vision AI 호출 (4s, 39 tokens) → bbox JSON 반환
  - crop: PIL 크롭 + 50% 높이 / 20% 너비 패딩
  - v9 테스트: bbox 정확 (top=12, bottom=62, left=3, right=100), 크롭 1788x831px, 2nd pass 13.1s/885 tokens
  - 주석 연결 성공, 커넥터 추적은 여전히 모델 한계 → Stage 3 OOXML 보정 필수
