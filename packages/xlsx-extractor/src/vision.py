...
SYSTEM_CONTEXT = """당신은 모바일 MMORPG "Project K"의 기획서 분석 전문가입니다. Excel 기획서 시트의 이미지를 분석하여
AI 지식 베이스에 저장할 수 있는 구조화된 텍스트로 변환합니다.

**프로젝트 배경**: 이 기획서의 모든 지식을 AI가 이해하여, 기획자의 QnA/리뷰/신규 기획을 실시간 보조하는 것이 목표입니다.
변환된 데이터는 (1) Markdown→벡터DB (설계 의도/맥락 검색), (2) 구조화 JSON→SQLite (정밀 수치 조회/비교/집계)로 저장됩니다.
...
def build_tile_prompt(sheet_name, tile_info, total_tiles, is_single=False,
                      tile_index=0, previous_md="", is_last_tile=False):
...
### 8. 테이블 구조 식별 (기조 데이터 추출용)
수치 데이터가 포함된 테이블을 발견하면, Markdown 테이블 외에 **추가로** ```table-structure 코드 블록을 출력하세요.
한 이미지에 테이블이 여러 개면 tables 배열에 각각 추가합니다.
수치 테이블이 없는 경우(플로우차트만, 텍스트 설명만) 이 블록은 생략합니다.
...
형식:
    ...
    {{
      "tables": [{{
        "table_id": "t1",
        "table_name": "테이블 제목 또는 설명",
        "header_row_approx": 11,
        "data_rows_approx": [12, 39],
        "columns_approx": ["컬럼명1", "컬럼명2", "..."],
        "notes": "이 테이블에 대한 부가 설명 (주석, 우측 메모 등)"
      }}]
    }}
    ```

규칙:
- header_row_approx, data_rows_approx는 Excel 행 번호의 **대략적 추정치** (정확하지 않아도 됨)
- columns_approx는 눈에 보이는 열 헤더 텍스트를 순서대로 나열
- 병합 헤더(다단 헤더)가 있으면 "상위헤더/하위헤더" 형식으로 표기
- **정확한 숫자 값은 여기서 필요하지 않음** — 구조(테이블 경계, 헤더, 컬럼명)만 식별
- 테이블이 이미지 경계에서 잘려 있으면, 보이는 부분만 출력"""
...
### 9. 지식화 설문 (중요)
이 시트의 모든 타일을 분석한 종합적 관점에서, ```survey 코드 블록에 아래 질문에 답하세요.
이전 섹션의 해석 결과와 현재 타일을 종합하여, 시트 전체의 지식적 가치를 평가해주세요.

    ```survey
    {
      "extraction_strategy": {
        "data_types_found": ["테이블", "수식", "차트", "플로우차트", "텍스트 설명" 등 해당되는 것],
        "key_information": "이 시트에서 가장 중요한 정보 3가지 (한 줄씩)",
        "hidden_insights": "표면적으로 드러나지 않지만 추출해야 할 암묵적 정보 (예: 셀 색상의 의미, 행간 배치 패턴, 주석의 설계 의도 등)",
        "best_format": "이 데이터를 어떤 형식으로 저장하면 가장 활용도가 높을까 (Markdown/JSON테이블/그래프/관계형 등)",
        "extraction_challenges": "추출 시 주의할 점, 놓치기 쉬운 부분"
      },
      "service_value": {
        "answerable_questions": ["이 데이터로 답할 수 있는 구체적 질문 예시 5개"],
        "cross_references": ["이 시트가 참조하거나 연관되는 다른 시스템/기획서 이름"],
        "planner_pain_points": "기획자가 이 데이터에 대해 가장 자주 궁금해할 포인트",
        "improvement_suggestions": "현재 변환 방식(Markdown + 구조화 JSON)을 개선할 수 있는 제안"
      }
    }
    ```"""

    prompt += survey_section
...
- 순서는 위 요소 목록 순서와 동일
- 각 요소를 **완전히** 포함하는 바운딩 박스를 지정 — 요소의 일부가 잘리면 안 됩니다
- 캐릭터/인물 이미지라면 머리 꼭대기부터 발끝까지, 주변 이펙트(오라, 마법 효과 등)도 포함
- 여백은 최소화: 요소 주변의 빈 공간(흰색/단색 배경)은 바운딩 박스에서 제외
- 이미지 경계에 걸쳐 있다면 보이는 부분의 끝까지 포함
...
def _expand_to_content(full_img, left, right, top, bottom, bg_threshold=240, min_content_ratio=0.01):
    """Vision bbox가 콘텐츠를 잘라냈을 때 상하로 확장하여 보정.
    해당 열 범위(left~right)에서 위/아래로 콘텐츠가 있으면 경계를 확장."""
    import numpy as np
    arr = np.array(full_img.convert("RGB"))
    H, W = arr.shape[:2]

    # 해당 열 범위의 슬라이스
    col_slice = arr[:, left:right, :]
    col_w = right - left
    if col_w <= 0:
        return top, bottom

    non_bg = ~np.all(col_slice > bg_threshold, axis=2)
    row_density = non_bg.sum(axis=1) / col_w

    content_rows = np.where(row_density > min_content_ratio)[0]
    ...
    # 콘텐츠가 bbox 밖에 있으면 확장 (최대 bbox 높이의 50%까지)
    box_h = bottom - top
    max_expand = int(box_h * 0.5)

    new_top = top
    if content_top < top:
        new_top = max(content_top - 5, top - max_expand, 0)

    new_bottom = bottom
    if content_bottom > bottom:
        new_bottom = min(content_bottom + 5, bottom + max_expand, H)

    return new_top, new_bottom


def _auto_trim_whitespace(img, bg_threshold=240, min_content_ratio=0.005, padding=5):
    """서브이미지의 상하좌우 빈 여백(흰색/밝은 배경)을 자동 트림.
    Vision의 바운딩 박스가 부정확해도 결과물이 깔끔하게 나오도록 보정."""
    import numpy as np
    ...
    # 행별/열별 콘텐츠 비율
    row_density = non_bg.sum(axis=1) / W
    col_density = non_bg.sum(axis=0) / H
    ...
    # 트림이 의미 있을 때만 (10% 이상 줄어드는 경우)
    trimmed_area = (right - left) * (bottom - top)
    original_area = W * H
    if trimmed_area < original_area * 0.90:
...
def extract_sub_images(tile_text, tile_image_path, output_dir, tile_id, sheet_safe_name):
    ...
    for i, match in enumerate(matches):
        ...
        if bbox:
            ...
            left_pct = max(0, bbox["left"] - pad_h)
            right_pct = min(100, bbox["right"] + pad_h)
            top_pct = max(0, bbox["top"] - pad_v)
            bottom_pct = min(100, bbox["bottom"] + pad_v)
            ...
            # Vision bbox가 상단을 너무 낮게 잡는 경우 보정:
            # 해당 열 범위에서 위쪽으로 콘텐츠가 있으면 확장
            cropped_initial = img.crop((left, top, right, bottom))
            top, bottom = _expand_to_content(img, left, right, top, bottom)

            filename = f"{sheet_safe_name}_{tile_id}_fig{fig_counter}.png"
            filepath = os.path.join(images_dir, filename)
            cropped = img.crop((left, top, right, bottom))
            cropped = _auto_trim_whitespace(cropped)
