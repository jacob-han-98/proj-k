...
- **분석 절차** (반드시 이 순서를 따르세요):
  ...
  2. 각 도형에서 나가는 선을 하나하나 추적:
     - 해당 도형의 **외곽선에서 나가는 선**이 몇 개인지 확인
     - 각 선이 나가는 **방향**(오른쪽/위/아래/왼쪽)을 기록
     - 각 선을 **끝까지** 따라감 (꺾이거나 방향이 바뀌어도 끊기지 않으면 계속)
     - 선이 다른 선과 합류하면, 합류 후 **화살촉 방향**으로 계속
     - **화살촉이 닿는 도형**이 최종 도착지
  3. 결과를 테이블로 기록: | 출발 | 도착 | 라벨 |
  4. 테이블 기반으로 Mermaid flowchart 코드 작성
- **주의**: 도형에서 나가는 선이 2개 이상일 수 있음! 특히 긴 수평선/수직선이 이미지 끝까지 이어지는 경우를 놓치지 마세요
- 노드 목록과 화살표 분석을 `<!-- 화살표 분석 -->` 주석 블록에 기록 후 Mermaid 작성
- **절대 금지**: 이미지에 없는 화살표를 추측하여 추가하지 마세요
- **주변 주석/참조 텍스트 연결**: 플로우차트 노드 근처의 주석 텍스트는 **가장 가까운 노드의 주석**으로 기록:
  - Mermaid 코드 블록 바로 아래에 `> **[노드명] 주석**: 내용` 형식
